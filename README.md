...
- json_files: a folder that stores all the outputs in several subfolders and a json file:
  ...
  - test: contains all kinds of json files of the test set
    ...
    + transcription_output: a folder that stores the raw and processed transcriptions generated by the Whisper model
    + TP_to_FN.json (extracted instances that were actually True Positives(being correctly classified) of the unintelligible class, but turned True Negatives after applying the confidence threshold)
    ...
    + real_false_negatives.json (stores all FN that were still FN before applying the confidence threshold)
    + test_NAs_info.json (a json file for storing nonsensical transcriptions of the test set)
    + test_metrics_output.json (results of the test)
  ...
  - transcription_output: a folder stores all the outputs of the dev set 
    ...
    + raw_tanscription_info: a folder that contains the raw transcription data generated by the Whisper model across different temperature settings
    
    + raw_text: a folder that contains the raw transcription (only the text part) across different temperature settings 
